...
# driving Lj with Ai
Lj_2_car_mode <- Lj_wAi_wMode(in_car_matrix, -0.02310491, "COL38", "mode_car", Ai_1_car)
# Lj transit with Ai
sched_matrices <- list.files(schedule_dir)
access_G_list <- list()
f <- 1
for (sm in sched_matrices) {
  ...
  sm_name <- paste(schedule_dir, '/', sm, sep='')
  in_transit_matrix <- read.csv(sm_name, check.names=FALSE)
  out_matrix <- Lj_wAi_wMode(in_transit_matrix, -0.02310491, "COL38", "mode_transit", Ai_1_transit)
  access_G_list[[f]] <- out_matrix
  f <- f + 1
}
# averaging the initial transit
...
for (sm in sched_matrices) {
  ...
  sm_name <- paste(schedule_dir, '/', sm, sep='')
  in_transit_matrix <- read.csv(sm_name, check.names=FALSE)
  out_matrix <- Ai_wLj(in_transit_matrix, -0.02310491, emp_col,Lj_2_mode)
  access_G_list[[f]] <- out_matrix
  f <- f + 1
}
# averaging the initial transit
...
# driving Ai with Lj
Ai_2_car <- Ai_wLj(in_car_matrix, -0.02310491, emp_col, Lj_2_mode)
Ai_2_car[Ai_2_car < 0.001] <- 0.001


out_a <- merge(Ai_2_transit,Ai_2_car,by.x = 0, by.y = 0)
write.csv(out_a,file = out_file_name)


# # #
Ai_transit_merge <- merge(Ai_initial_transit,Ai_1_transit,by.x = 0,by.y = 0)
Ai_transit_merge <- merge(Ai_transit_merge,Ai_2_transit,by.x = 1,by.y = 0)
colnames(Ai_transit_merge) <- c("dauid","Ai_t_1","Ai_t_2","Ai_t_3")
...
print(mean(abs(Ai_transit_Z$Ai_t_2-Ai_transit_Z$Ai_t_3)))


# # #
Ai_car_merge <- merge(Ai_initial_car,Ai_1_car,by.x = 0,by.y = 0)
Ai_car_merge <- merge(Ai_car_merge,Ai_2_car,by.x = 1,by.y = 0)
colnames(Ai_car_merge) <- c("dauid","Ai_c_1","Ai_c_2","Ai_c_3")
...
print(mean(abs(Ai_car_Z$Ai_c_2-Ai_car_Z$Ai_c_3)))
